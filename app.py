...
def fetch_college_data():
    """Fetch and parse data from college website"""
    ...
    try:
        ...
        if title:
            data_parts.append(f"Title: {title.get_text().strip()}")
        
        # Get headings (h1, h2, h3)
        for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
            text = tag.get_text().strip()
            if text and len(text) > 3:
                data_parts.append(f"Heading: {text}")
        ...
        for nav in soup.find_all('nav'):
            for a in nav.find_all('a'):
                ...
                if text and len(text) > 2:
                    data_parts.append(f"Link: {text}")
        
        # Get meta descriptions
        for meta in soup.find_all('meta'):
            desc = meta.get('content')
            if desc and len(desc) > 20:
                data_parts.append(desc)
        ...
        college_data_cache = "\n\n".join(cleaned_data[:50])  # Limit to 50 parts
        last_fetch_time = time.time()
        
        print(f"✅ Fetched college data: {len(cleaned_data)} content sections")
        return college_data_cache
